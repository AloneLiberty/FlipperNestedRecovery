...
void SendCommandOLD(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2, void *data, size_t len) {
    ...
    /**
    This causes hangups at times, when the pm3_old unit is unresponsive or disconnected. The main console thread is alive,
...
/**
 * @brief This method should be called when sending a new command to the pm3_old. In case any old
    ...
    switch (packet->cmd) {
        ...
        case CMD_DEBUG_PRINT_STRING: {
            ...
            if (flag & FLAG_LOG) {
                ...
                //PrintAndLogEx(NORMAL, "[" _MAGENTA_("pm3_old") "] ["_BLUE_("#")"] " "%s", s);
        ...
        case CMD_DEBUG_PRINT_INTEGERS: {
            if (packet->ng == false)
                PrintAndLogEx(NORMAL, "[" _MAGENTA_("pm3_old") "] ["_BLUE_("#")"] " "%" PRIx64 ", %" PRIx64 ", %" PRIx64 "", packet->oldarg[0], packet->oldarg[1], packet->oldarg[2]);
            break;
        }
...
bool OpenProxmark(pm3_device_t **dev, const char *port, bool wait_for_port, int timeout, bool flash_mode, uint32_t speed) {
    ...
    if (sp == INVALID_SERIAL_PORT) {
        ...
        PrintAndLogEx(HINT, "Try the shell script " _YELLOW_("`./pm3_old --list`") " to get a list of possible serial ports");
        sp = NULL;
        return false;
...
int TestProxmark(pm3_device_t *dev) {
    ...
    // as this is the very first command sent to the pm3_old
...
//            PrintAndLogEx(INFO, "Waiting for a response from the Proxmark3...");
            PrintAndLogEx(INFO, "You can cancel this operation by pressing the pm3_old button");
            show_warning = false;
        }
